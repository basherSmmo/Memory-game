...
let rezise = new ResizeObserver((ent) => {
    ...
    blocks.style.setProperty("grid-template-columns",`repeat(auto-fit, minmax(${Math.sqrt((ent[0].contentRect.height * ent[0].contentRect.width) / (numberOfImages * mu)) + "px"}, auto))`); 
})
rezise.observe(blocks)
function setBlocks() {
    if(blocks.childElementCount !== 0) blocks.innerHTML = "";
    for(let i = 0, count = []; numberOfImages !== count.length && i < 2;) {
    ...
    cards.forEach((e) => {
        e.onclick = () => {
            if(forCompare.length < 2) {
                ...
                forCompare.push(e);
                if(forCompare.length == 2) {
                    if(forCompare[0].lastElementChild.src === forCompare[1].lastElementChild.src) {
                        forCompare = [];
                        complete--;
                    } else {
                        setTimeout(() => {
                ...
                if(complete === 0) {
                    ...
                    result.innerHTML = `Wrong tries ${numberOfTries} | Time ${time.toLocaleTimeString().slice(2,7)}`
